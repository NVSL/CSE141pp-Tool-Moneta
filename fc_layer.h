...
#include "layer_t.h"
	...
	int map( point_t d, tensor_t<float> & in )
	{
	...
	extern "C" void test_activate_fc( float* a, float* b, tensor_t<float>&in, tensor_t<float>&out, tensor_t<float> &weights, std::vector<float> & input) {
...
struct fc_layer_t
{
	...
	void activate()
	{
		test_activate_fc(&in(0, 0, 0), &in(in.size.x-1, in.size.y-1, in.size.z-1), in, out, weights, input);
		/*for ( int n = 0; n < out.size.x; n++ )
		{
			float inputv = 0;
