    ...
    def create_tools(self):
        ...
        if 1:  # tool_select:
            ...
            def undo(*args):
                ...
                self.curr_action = Action.undo
                (x1, x2), (y1, y2) = self.undo_actions[-1]
                with self.scale_x.hold_trait_notifications():
                    with self.scale_y.hold_trait_notifications():
                        self.scale_x.min, self.scale_x.max = x1, x2
                        self.scale_y.min, self.scale_y.max = y1, y2
            self.undo_btn.on_event('click', lambda *ignore: undo())
            ...
            def redo(*args):
                print("redoing")
                print(self.redo_actions)
                self.curr_action = Action.redo
                (x1, x2), (y1, y2) = self.redo_actions[-1]
                with self.scale_x.hold_trait_notifications():
                    with self.scale_y.hold_trait_notifications():
                        self.scale_x.min, self.scale_x.max = x1, x2
                        self.scale_y.min, self.scale_y.max = y1, y2
            self.redo_btn.on_event('click', lambda *ignore: redo())
