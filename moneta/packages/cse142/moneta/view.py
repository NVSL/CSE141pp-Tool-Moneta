...
from cse142.settings import CUSTOM_CMAP, INDEX_LABEL, ADDRESS_LABEL, INDEX, ADDRESS, HISTORY_MAX, MONETA_OUTPUT_DIR, CWD_HISTORY_FILE_NAME
...
from cse142.moneta.moneta_widgets import MonetaWidgets
...
class View():
    ...
    def update_cwd_widget(self, cwd_path):
        if not cwd_path in (".", "./") and not cwd_path in self.m_widget.cwd.options:
            ...
            update_cwd_file(self.m_widget.cwd.options, MONETA_OUTPUT_DIR, CWD_HISTORY_FILE_NAME)
    ...
    def handle_generate_trace(self, _):
        ...
        if generate_trace(w_vals, tool='moneta'):
