...
sys.path.append(BASE_DIR + "moneta/moneta/")
...
def load_cwd_file(out_dir, file):
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)
        log.debug(f"Making output dir")
    try:
        with open(f"{out_dir}/{file}", "a+") as history:
            history.seek(0)
...
def update_cwd_file(cwd_history, out_dir, file):
    with open(f"{out_dir}/{file}", "w+") as history_file:
...
def verify_input(w_vals):
    ...
    if (
        ('c_lines' in w_vals and w_vals['c_lines'] <= 0) or 
        ('c_block' in w_vals and w_vals['c_block'] <= 0) or 
        ('m_lines' in w_vals and w_vals['m_lines'] <= 0)
    ):
    ...
    if 'cwd_path' in w_vals and not os.path.isdir(w_vals['cwd_path']):
...
def widget_vals_to_args(w_vals, tool='moneta'):
    if tool == 'moneta':
        # Pin double-open error fix
        if(os.path.isfile(MONETA_OUTPUT_DIR + "trace_" + w_vals['o_name'] + ".hdf5")):
            subprocess.run(["rm", MONETA_OUTPUT_DIR + "trace_" + w_vals['o_name'] + ".hdf5"])

        return [
            PIN_PATH, "-ifeellucky", "-injection", "child", "-t", MONETA_TOOL_PATH,
            "-name", w_vals['o_name'],
        ...
        return [
            PIN_PATH, "-ifeellucky", "-injection", "child", "-t", CFG_TOOL_PATH,
            "--", w_vals['e_file'], *w_vals['e_args']
        ]
...
def run_pintool(w_vals, tool='moneta'):
    log.info("Running pintool")
  
    args = widget_vals_to_args(w_vals, tool)

    if not args:
        print(f"{TextStyle.RED}Invalid Pintool Type: {tool}. "
              f"Valid Types: moneta, cfg {TextStyle.END}")
        return False
...
def generate_trace(w_vals, tool='moneta'):
    if verify_input(w_vals) and run_pintool(w_vals, tool):
        border_char = '#'
        side = border_char * 5
...
def generate_cfg(edges, outfile_name):
    ...
    dot.render(outfile_name, view=True)