...
class PlotStats():
    def __init__(self, model):
        ...
        self.total_mem_accessed = Label(value='Cache Lines: Loading...')
        self.total_stats = VBox(
                           [self.total_stat_title, self.total_hits, self.total_cap_misses, self.total_comp_misses, self.total_mem_accessed],
        ...
        self.curr_bytes = Label(value='Bytes: Loading...')
        self.curr_mem_accessed = Label(value='Cache Lines: Loading...')
        self.curr_stats = VBox(
                          [self.curr_stat_title, self.curr_hits, self.curr_cap_misses, self.curr_comp_misses, self.curr_mem_accessed],
                          layout=Layout(width='250px', padding="5px")
                          )
    ...
    def update(self, init=False):
        total_count, hit_count, cap_miss_count, comp_miss_count, lines, bytes = self.get_curr_stats()
        ...
        if(init):
            ...
            self.total_mem_accessed.value = stats_string(STATS_MEM_ACCESSED, mem_accessed_stats(bytes, lines, self.model.curr_trace.cache_lines))
    ...
    def get_curr_stats(self):
        ...
        return total_count, hit_count, cap_miss_count, comp_miss_count, lines, bytes
