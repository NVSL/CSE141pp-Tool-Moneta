...
extern "C" void writeData(unsigned long long addr, bool isWrite){
	...
	data[numAccess].address = addr;
	data[numAccess].write = isWrite;
...
//	std::cout<<"writing: "<<*data[NX-1];	
	...
	status = H5Dwrite(dataset, dtype, dataspace, filespace, H5P_DEFAULT, data);
...
extern "C" void createFile(){
	...
	dtype = H5Tcreate(H5T_COMPOUND, sizeof(access_t));
	H5Tinsert(dtype,"address", HOFFSET(access_t, address), H5T_NATIVE_LLONG);
	H5Tinsert(dtype, "isWrite", HOFFSET(access_t, write), H5T_NATIVE_CHAR);
	...
	dataset = H5Dcreate2(file, DATASETNAME, dtype, dataspace,H5P_DEFAULT, cparms, H5P_DEFAULT);
